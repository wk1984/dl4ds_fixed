...
ARG url0=https://github.com/conda-forge/miniforge/releases/download/4.14.0-0/Miniforge3-4.14.0-0-Linux-x86_64.sh
...
RUN if [ "$DL4DS" = true ]; then \
    echo "install DL4DS ..."; \
    . /root/.bashrc; \ 
    mamba create -n dl4ds_py39_cu11 -c conda-forge python==3.9.* xarray cartopy cudatoolkit==11.* cudnn==8.* numpy==1.* g++==11.* -y; \
